    ...
    try:
        with nd.open(serial=serial, configuration=config) as device:
            ...
            for status, packet in device:
                if packet:
                    #Check Length of Packet and randomly drop events in packet until it is under the required packet size
                    #TODO do some testing to identify what is the maximum event rate/packet size the frame gen can handle
                    #TODO check what the time cost of the outside of the for loop is
...
class cameraManager:
    def __init__(self, idx, eventMemory:shared_memory.SharedMemory, irMemory:shared_memory.SharedMemory, eventCameraShape:tuple, irCameraShape:tuple):
        self._idx = idx
    ...
    async def get_frame(self):
        ...
        mergedFrame = np.concatenate((rgbEventFrame, rgbIRFrame), 0)

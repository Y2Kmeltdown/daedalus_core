...
def readAtmos(ctrl_meas_WORD, ctrl_hum_WORD, config_WORD, measurement_T):
    i2cbus.write_byte_data(r_dict["i2c_address"],r_dict["reset_REG"],reset_WORD)

    i2cbus.write_byte_data(r_dict["i2c_address"],r_dict["config_REG"],config_WORD)

    i2cbus.write_byte_data(r_dict["i2c_address"],r_dict["ctrl_hum_REG"],ctrl_hum_WORD)
    i2cbus.write_byte_data(r_dict["i2c_address"],r_dict["ctrl_meas_REG"],ctrl_meas_WORD)
    
    while(True):
        status_data = i2cbus.read_byte_data(r_dict["i2c_address"],r_dict["status_REG"])
        #print(bin(status_data))
        time.sleep(measurement_T)
        if status_data == 0:
            pressure = i2cbus.read_i2c_block_data(r_dict["i2c_address"], r_dict["press_msb_REG"], 3)
            pressureBinary = "".join([format(val, '#010b')[2:] for val in pressure])[0:-4]
            pressureInt = int(pressureBinary, 2)
            #  perform compensation
            
            
            temperature = i2cbus.read_i2c_block_data(r_dict["i2c_address"], r_dict["temp_msb_REG"], 3)
            temperatureBinary = "".join([format(val, '#010b')[2:] for val in temperature])[0:-4]
            temperatureInt = int(temperatureBinary, 2)
            #  perform compensation
            

            humidity = i2cbus.read_i2c_block_data(r_dict["i2c_address"], r_dict["hum_msb_REG"], 2)
...
if __name__ == "__main__":
    ...
    readAtmos(ctrl_meas_WORD, ctrl_hum_WORD, config_WORD, measurement_T)

...
static PyObject* get_camera_buffer(PyObject *self, PyObject *args, PyObject *kwargs) {
        ...
        if (ARV_IS_BUFFER(buffer)) {
            ...
            if (raw) {
                ...
                if (isRaw) {
                    ...
                    memcpy(array_data, raw, buffer_sz);
                }
                else {
                    ...
                    if (!stretched) {
                        ...
                        //array = PyBytes_FromStringAndSize(stretched, npix);
                        //free(stretched);
                    }
                }
                
            } else {
                array = PyUnicode_FromString("No data available");
            }
        } else {
            array = PyUnicode_FromString("Failed to acquire buffer");
        }
        
        /* Destroy the buffer */
        g_clear_object(&buffer);
    } else {
        array = PyUnicode_FromString("No camera found");
    }
...
static PyObject* ir_buffer_streamer(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    // Define keyword argument names
    static char *kwlist[] = {"raw", "lowFPS", NULL};
    ...
    int is30fps = 0;
    if (fpsCheck != NULL) {
        int result = PyObject_IsTrue(fpsCheck);
